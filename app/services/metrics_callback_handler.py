...
class MetricsCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response, **kwargs):
        ...
        chain_type = self.metadata.chain_type.value if self.metadata else "unknown"
        total_token_usage = 0
        ...
        if hasattr(response, 'llm_output') \
            ...
            and 'token_usage' in response.llm_output:
                ...
                total_token_usage = token_usage.total_tokens if token_usage else 0
                prompt_token_usage = token_usage.prompt_tokens if token_usage else 0
                completion_token_usage = token_usage.completion_tokens if token_usage else 0
        if "usage" in response and "model" in response:
        ...
        total_token_usage_counter.labels(
            type=chain_type, 
            name=chain_name, 
            group_id=group_id, 
            model=model_name).inc(total_token_usage)
        prompt_token_usage_counter.labels(
            type=chain_type, 
            name=chain_name, 
            group_id=group_id, 
            model=model_name).inc(prompt_token_usage)
        completion_token_usage_counter.labels(
            type=chain_type, 
            name=chain_name, 
            group_id=group_id, 
            model=model_name).inc(completion_token_usage)
